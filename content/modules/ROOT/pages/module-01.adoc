...
.. remove `None` and type `PCI`
.. Select `PCI-DSS v4.0 Control Baseline for Red Hat Enterprise Linux 8`
.. Review the information on screen.  This will define the profile and show some of the appropriate packages, file system configurations, kernel arguments, and services that will be added to the image.
.. Click `Next`
. `File system configuration`
.. You can use automatic partitioning or you can manually configure the partitions.  
.. In a previous step if a SCAP profile wasn't selected, then you will need to select `Manually configure partitions`.  This opens up a new table where you can add additional partitions.
.. Click `Add partition`
.. Set `/home` to '5 GiB`.
.. Click `Add partition`
...
.. The `Chosen packages` list now contains `postgresql-server` and `openscap`.  It will also contain any selections from the OpenSCAP policy that was selected.
